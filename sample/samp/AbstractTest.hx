...
//			log("AbstractTest: publish cmd"+cmd);
        ...
        public function openChannel(_callback:Dynamic):Void {
            ...
            var devNull:Dynamic = function(event:ProtocolEvent):Void{
                _this.log("devNull called for " + event.command.method);
            };
		...
		public function openChannel2(_callback:Dynamic):Void {
            ...
            sessionHandler2.rpc(new Command(open), devNull);
            sessionHandler2.rpc(new Command(exchange), devNull);
            sessionHandler2.rpc(new Command(queue), devNull);
            sessionHandler2.rpc(new Command(bind), _callback);//addAsync(_callback, TIMEOUT));
        }
